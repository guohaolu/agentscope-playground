...
### Notebook 与外部进程的稳定性规则

如果 notebook 需要调用以下这类能力：

- MCP 客户端
- 浏览器自动化
- 需要启动外部子进程的工具
- 在 Windows 下容易受到事件循环影响的异步能力

则不要优先在 notebook 单元中直接长时间持有这类连接或直接 `await` 启动复杂外部进程。

更稳的默认方案是：

1. notebook 继续负责中文讲解、步骤引导、结果展示
2. 真正的外部进程连接逻辑放进单独的 `.py` helper
3. notebook 通过普通 Python 函数调用 helper
4. helper 内部自己管理独立事件循环
5. 如果 notebook 环境里已经存在运行中的事件循环，则 helper 应在独立线程中运行自己的事件循环，而不是直接调用 `asyncio.run()`

原因：

- Windows + Jupyter + 异步子进程的兼容性不稳定
- 直接在 notebook 中运行 MCP/浏览器子进程，容易出现事件循环相关报错
- 使用 helper 可以降低 notebook 环境差异带来的问题

如果采用这种方式，必须在 notebook 中明确解释：

- 为什么不直接在 notebook 里连接
- helper 解决了什么稳定性问题
- notebook 和 helper 的职责边界是什么

对于浏览器自动化、MCP 浏览器客户端、Playwright MCP 这类场景，进一步采用下面的默认约定：

- notebook 负责学习说明、概念讲解、运行指引、结果解读
- 真正执行浏览器操作时，优先提供可直接运行的 `.py` 脚本
- 脚本负责连接 MCP、执行浏览器动作、打印结果、保存截图
- 后续的点击、输入、搜索等扩展，也优先继续基于 `.py` 脚本演进
...
除非用户明确要求必须在 notebook 内直接执行，否则不要把浏览器沙盒的真实执行主流程放在 notebook 单元里。
